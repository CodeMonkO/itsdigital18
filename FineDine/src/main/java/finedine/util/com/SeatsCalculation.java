...
public class SeatsCalculation {
	public ModelMap getSeats(RestaurantLiveEntity restaurantLiveEntity,
			Booking bookingform, ModelMap model) {
		int vacantSeats = Integer.parseInt(restaurantLiveEntity.getMaxseat())
				- Integer.parseInt(restaurantLiveEntity.getBookedseat());
		int bookedseats = Integer
		...
		if (Integer.parseInt(bookingform.getBooking()) <= (maxseats - bookedseats)) {
			System.out
					.println("hello :" + restaurantLiveEntity.getBookedseat());
			model.addAttribute("bookedseats",
					bookedseats + Integer.parseInt(bookingform.getBooking()));
			model.addAttribute("maxseats", maxseats);
			model.addAttribute("vacantseats",
					vacantSeats - Integer.parseInt(bookingform.getBooking()));
		} else {
	...
	public ModelAndView getSeats(RestaurantLiveEntity restaurantLiveEntity,
			ModelAndView model) {
		int vacantSeats = Integer.parseInt(restaurantLiveEntity.getMaxseat())
				- Integer.parseInt(restaurantLiveEntity.getBookedseat());
		int bookedseats = Integer
